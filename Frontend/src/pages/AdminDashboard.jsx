...
import { BarChart3, Users, Hotel, DollarSign, Calendar, Settings, Building, Bed } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const handleRoomTypeUpdate = async (roomTypeId, roomTypeData) => {
    try {
      const response = await bookingsService.updateRoomType(roomTypeId, roomTypeData);
      toast.success('Room type updated successfully');
      setRoomTypes(prev => prev.map(roomType => 
        roomType.id === roomTypeId ? { ...roomType, ...roomTypeData } : roomType
      ));
      return response;
    } catch (error) {
                      ...
                      <div key={booking.id} className="flex items-center justify-between py-2">
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {booking.room_type} • {formatCurrency(booking.total_price)}
                          </div>
                        </div>
